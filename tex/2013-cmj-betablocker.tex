...
Computation is odd. 
In fact in many ways it is one of the strangest things we have discovered, and 77 years on we continue to fail to fully grasp its behaviour.
% TODO forDave: Reviewer comment: Computers and scripts that run them, hardware and software are a man made creation these come from particular social political desires and circumstances. People programme the computer, they didn't just evolve like a rock that forms from 'geophysical teutonics' so in this case, this paragraph should be revised or expanded. Till: what should we do with this? I actually think that computation _is_ discovered and quite distinct to computers and scripts. but I don't want to elaborate on this at this very point, for the sake of the point to be made.
...
Livecoding attempts to treat computation as a material on its own terms, to pick it up and try to gain understanding into what it looks\citep{mclean2010-vis} and sounds like.

Based on these grounds, we introduce Betablocker, our contribution to the field of artistic understanding of low level computation \citep{2006-the},\footnote{Viznut ``Algorithmic symphonies from one line of code'' \url{http://countercomplex.blogspot.co.uk/2011/10/algorithmic-symphonies-from-one-line-of.html}} a field that digital sound synthesis research discovered as early as in the 1970-ies \citep{berg2009-com}.
Betablocker intersects livecoding practise, and brings to light many different strategies for presenting computation as tangible material -- by making \emph{process} as much part of livecoding as the code that describes it.
...
This allows for alteration regarding introspection (by adding probes) and customisation (by altering the instruction set).
...
% TODO forDave: reviewer comment: use images to describe [...] the 'new ideas for artistic use'.
...
% TODO forDave: editors comment: as a suggestion, reference one of the cognitive dimensions of notation here, which might provide further dimensions along which success (or otherwise) could be measured/considered.
...
Due to its complexity a valid and, as it turned out, fruitful approach is to work with randomness and empirical observations rather than with analytical approaches.
...
% TODO forDave: reviewer comment: If you need to cut, the section discussing the various performances could probably be made more concise, and/or focus on the most interesting cases. till: the other reviewer wants to have less a technical level, so I guess this renders irrelevant.
...
\label{sub:piksel}
% TODO forDave: reviewer comment: As a reader I would like to have an indication of what and how the 'performer' performed, f the gamepad was the only input was there gestures moving to the a/v what was the 'dramaturgical relationship of the live code and this relationship to the interface'? if indeed it was a performance was the 'audience' affect considered or what was the 'response' or atmosphere created by the live performance event.
% TODO forDave: editor comment: tell more about the first performance.
...
% TODO reviewer comment: Would be great to hear the 'greater sonic possibilities'.
