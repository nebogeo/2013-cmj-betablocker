...
It is designed to follow the \emph{von Neumann architecture} i.e., that there is no separation between program code and data.~\cite{von-neumann1993-fir}
...
The heap is where all data are stored.
Every combination of values is a valid input to Betablocker, and each of these value is interpreted either as an instruction, an address, or a data item.
In line with the von Neumann architecture, it is possible (and intended) to alter the heap while it is running.
This especially means that what might be considered as the "program" is possibly being altered as well.

% instruction set
Regarding the instruction table, we differentiate between the core instruction set consisting of 22 distinct instructions\footnote{See Appendix for a list of all isntructions} and various extensions to this set, which, depending on the actual application, introduce features like playing a note (\texttt{NOTE}) or copying information to the output pins of an AVR chip (\texttt{OUT}).
As there is no instruction telling the engine to halt, programs will continue to run until externally terminated.
However, they will eventually converge into a (possibly complex) stable state.

% thread
A thread is the part that executes code stored on the heap. 
Its program counter $pC$ stores the current position (i.e., the instruction it is currently evaluating).
A stack finally serves as a temporary storage for a thread. 
It can be accessed only from top by either \emph{popping} an item from the stack (e.g.~\texttt{POP}), or pushing a value to the stack (e.g.~\texttt{PSH}).
...
Betablocker came to live as the first attempt at a game pad programmable live coding system in fluxus.\footnote{
...
Betablocker originally was a virtual machine running inside fluxus.
It visualised the simulation of a fictional CPU with 256 bytes of memory, and allowed multiple threads of execution to be run sharing the same address space. 
The Betablocker language is very much based on the ideas in Core Wars.\footnote{
...
\subsubsection{Genetic programming} % (fold)
\label{sub:genetic_programming}

An introduction to genetic programming with Betablocker that explains its value regarding lifecoding practice.
E.g. a method to find interesting patterns/sounds before doing a livecoding performance.
