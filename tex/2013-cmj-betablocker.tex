...
Computation is odd. 77 years on we continue to fail to fully grasp its behaviour.
...
The notions of code, data and program are therefore synonyms, however they represent semantic meaning rather than systematical/structural meaning.
...
The use of Genetic Algorithms \citep{barricelli1963numerical} in this livecoding case is different from their traditional use. In the case of Betablocker/Rhythm the desire to use genetic algorithms arose from the need to discover and learn new, optimal coding patterns for use in livecoding performances, as a way of escaping a feeling of being `stuck in a creative rut'. The use of a genetic algorithm in this case it to provide \emph{new ideas for musical use} - not final programs for their own sake.
...
The very first performance of Betablocker in the Fluxus environment took place in 2006 at the Piksel open source media arts festival in Norway. This was a 20 minute solo livecoding performance using a gamepad as the only input mechanism. Ring menus activated by analogue joysticks were used to pick instructions and set data values, while the directional buttons were used to navigate memory and choose addresses. A 16X16 zoomable grid of program/data was projected and both sample and synthesis patches were triggered; at least two full memory zero programs had to be run throughout the performance to get it back under musical control. Despite the esoteric nature of the assembly code the audience feedback implied that this was a particularly effective livecoding performance, to a large part due to the inclusion of the audio synchronised emph{movement of the process}, program counter, audio trigger animation and pointer visualisations - an early indication that understanding these mechanisms are important to an audience. Subsequently Betablocker was used in a series of performances in London, both solo and in collaborative musical live sessions as part of the group 'slub'\footnote{\url{http://slub.org}}.
